...
class NeuralNetwork:
    ...
    def __init__(self, X, Y, n_h=(5, 5), alpha=0.03,
                 param_init='he', regularization='L2', check_gradient=False):

        self.__X, self.__Y = X, Y
        self.__m = self.__X.shape[1]
        self.layer = len(n_h) + 1
        self.alpha = alpha

        self.n = np.empty(self.layer + 1, dtype=int)
        self.n[0] = self.__X.shape[0]
        self.n[self.layer] = self.__Y.shape[0]
        for (k, u) in zip(range(1, self.layer), n_h):
            self.n[k] = u
    ...
    def init_with_zeros(self, W, b):
        ...
        for i in range(1, W.shape[0]):
            W[i] = np.zeros(self.weight_shapes[i])
        for i in range(1, b.shape[0]):
            b[i] = np.zeros((self.n[i], 1))
        return W, b

    def init_with_random(self, W, b):
        """
        TODO
        :return:
        """
        ...
        for i in range(1, W.shape[0]):
            W[i] = np.random.randn(self.weight_shapes[i][0], self.weight_shapes[i][0]) * 10
        for i in range(1, b.shape[0]):
            b[i] = np.random.randn(self.n[i], 1) * 10
        return W, b

    def init_with_he(self, W, b):
        """
        TODO
        抑制梯度异常(梯度消失/梯度爆炸)初始化参数
        :return:
        """

        for i in range(1, W.shape[0]):
            W[i] = np.random.randn(self.weight_shapes[i][0], self.weight_shapes[i][1]) * np.sqrt(2 / self.n[i-1])
        for i in range(1, b.shape[0]):
            b[i] = np.random.randn(self.n[i], 1) * np.sqrt(2 / self.n[i-1])
        return W, b

    def init(self):
        """
        Random initialize the W and b with param_init method.

        :return: W, b.
        """
        W = np.empty(self.layer + 1, dtype=np.ndarray)
        b = np.empty(self.layer + 1, dtype=np.ndarray)
        if self.param_init == 'zeros':
            W, b = self.init_with_zeros(W, b)
        elif self.param_init == 'random':
            W, b = self.init_with_random(W, b)
        elif self.param_init == 'he':
            W, b = self.init_with_he(W, b)
        else:
            print(f"The initialization method excluding '{self.param_init}'")
