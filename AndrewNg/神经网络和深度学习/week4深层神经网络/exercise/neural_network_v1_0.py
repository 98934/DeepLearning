...
class NeuralNetwork:
    ...
    def __init__(self, X, Y, layer=3, units_hidden_layer=None, learning_rate=1.0):
        if units_hidden_layer is None:
            units_hidden_layer = [5]
        ...
        self.unit[0] = self.X.shape[1]
        self.unit[-1] = self.Y.shape[1]
        k = 1
        for u in self.units_hidden_layer:
            self.unit[k] = u
            k += 1

        self.theta_shapes = np.empty(self.layer - 1, dtype=tuple)
        for i in range(self.theta_shapes.shape[0]):
            self.theta_shapes[i] = (self.unit[i + 1], self.unit[i] + 1)
